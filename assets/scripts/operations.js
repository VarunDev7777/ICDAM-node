...
switch (pageparams){
    case "about" : changeactivelink(links[1])
    break;
    case "papers" : changeactivelink(links[2])
    break;
    case "special-sessions" : changeactivelink(links[3])
    break;
    case "registrations" : changeactivelink(links[4])
    break;
    case "sponsorship" : changeactivelink(links[5])
    break;
    case "publication" : changeactivelink(links[6])
    break;
    case "committe" : changeactivelink(links[7])
    break;
    case "conference-venue" : changeactivelink(links[8])
    break;
    case "downloads" : changeactivelink(links[9])
    break;
    case "previous-conference" : changeactivelink(links[10])
    break;
    case "" : changeactivelink(links[0])
    break;
}
...
let scroll_width = slider_con.scrollWidth
...
function changeactiveItem(){
    ...
    if(activeobj.nextElementSibling){
        activeobj.nextElementSibling.classList.replace("inactive-citem","active-citem");
    }
    else{
...
if(pageparams == "registrations"){
    ...
    targetString.innerHTML = targetString.textContent.replace(/\S/g, "<span class='letter'>$&</span>");
